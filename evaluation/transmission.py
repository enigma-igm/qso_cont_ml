...
class MeanTransmission(ModelResults):
    ...
    def __init__(self, testset, net, scaler_hybrid, n_iterations=100, interval=68., redshift_lims=(2.5,3.5)):
        ...
        self.mean_trans_pred = np.ma.array(trans_pred, mask=((trans_pred < 0.) | (trans_pred > 10.))).mean(axis=0)
        self.mean_trans_true = np.ma.array(trans_true, mask=((trans_true < 0.) | (trans_true > 10.))).mean(axis=0)
